...
ThirdPersonCamera::ThirdPersonCamera(Context *context) : LogicComponent(context),
	...
	minfollowangle_(0), maxfollowangle_(0),
...
void ThirdPersonCamera::Update(float dt)
{
	...
	if(!allowpitch_)
	{
		float i=(curfollow_-minfollow_)/(maxfollow_-minfollow_);
		camangle_=minfollowangle_+(maxfollowangle_-minfollowangle_)*i;
	}
	...
	if(clipcamera_)
	{
		...
		//Log::Write(LOG_INFO, String(f)+String(" ")+String(curfollow_));
	}
...
void ThirdPersonCamera::HandleMain(StringHash eventType, VariantMap &eventData)
{
	static StringHash sender("sender"), offset("offset");
	Node *s = (Node *)eventData[sender].GetVoidPtr();
	if(s)
	{
		maintarget_ = WeakPtr<Node>(s);
		offset_=eventData[offset].GetFloat();
		//Log::Write(LOG_INFO, "Main target set.");
	}
	//else Log::Write(LOG_INFO, "No main target set.");
}

void ThirdPersonCamera::HandlePush(StringHash eventType, VariantMap &eventData)
{
	static StringHash sender("sender"), offset("offset");
	Node *s = (Node *)eventData[sender].GetVoidPtr();
	if(s) targets_.Push(WeakPtr<Node>(s));
	offset_=eventData[offset].GetFloat();
}
...
void ThirdPersonCamera::Start()
{
	...
	SubscribeToEvent(StringHash(""), URHO3D_HANDLER(ThirdPersonCamera,HandleMain));
	SubscribeToEvent(StringHash("Push"), URHO3D_HANDLER(ThirdPersonCamera,HandlePush));
...
Vector2 ThirdPersonCamera::GetMouseGround()
{
	...
	Vector2 ground;
	if(PickGround(ground,mousepos.x_,mousepos.y_)) return ground;
	else return Vector2(-1,-1);
}

void ThirdPersonCamera::SetPosition(Vector3 pos)
{
	newpos_=pos_=pos;
}
